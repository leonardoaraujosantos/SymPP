...
// Conjugacy classes of G: a partition of the elements where a ~ b iff some g∈G
// has g·a·g⁻¹ = b. Each class is returned as a vector of its members; classes
// are computed from the full element list (small/medium groups).
//
// Reference: sympy.combinatorics PermutationGroup.conjugacy_classes.
[[nodiscard]] SYMPP_EXPORT std::vector<std::vector<Permutation>> conjugacy_classes(
    const PermutationGroup& g);
// Number of conjugacy classes (= conjugacy_classes(g).size()).
[[nodiscard]] SYMPP_EXPORT long num_conjugacy_classes(const PermutationGroup& g);

// Center Z(G): the elements commuting with every generator (equivalently, with
// all of G). Returned as a permutation group generated by those elements (the
// identity is always present).
//
// Reference: sympy.combinatorics PermutationGroup.center.
[[nodiscard]] SYMPP_EXPORT PermutationGroup group_center(const PermutationGroup& g);

// Derived (commutator) subgroup G' = [G, G]: the subgroup generated by all
// commutators a·b·a⁻¹·b⁻¹ (a, b ∈ G), grown by BFS closure.
//
// Reference: sympy.combinatorics PermutationGroup.derived_subgroup.
[[nodiscard]] SYMPP_EXPORT PermutationGroup derived_subgroup(const PermutationGroup& g);

// ----- Solvability & nilpotency ----------------------------------------------
// Whether G is solvable: the derived series G ▷ G' ▷ G'' ▷ … reaches the trivial
// group. Computed by iterating derived_subgroup until the order stops shrinking;
// solvable iff that stable order is 1. (For a simple non-abelian group such as
// A₅, the derived subgroup equals G, so the loop detects no shrink and returns
// false immediately.)
//
// Reference: sympy.combinatorics PermutationGroup.is_solvable.
[[nodiscard]] SYMPP_EXPORT bool is_solvable(const PermutationGroup& g);

// Whether G is nilpotent: the lower central series γ₁ = G, γ_{i+1} = [G, γ_i]
// reaches the trivial group. Computed by iterating the lower-central step
// (subgroup generated by commutators [g, h] with g ∈ G, h ∈ γ_i) until the order
// stabilizes; nilpotent iff it reaches order 1.
//
