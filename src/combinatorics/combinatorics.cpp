...
PermutationGroup normal_closure(const PermutationGroup& g,
                                const std::vector<Permutation>& generators) {
    // Smallest normal subgroup of G containing `generators`. BFS that closes the
    // seed set under (a) left multiplication by its own elements and (b)
    // conjugation by the generators of G — the latter enforces normality.
    if (generators.empty()) {
        return PermutationGroup({Permutation::identity(g.degree())});
    }

    // Build a conjugation-closed generating set: starting from `generators`, add
    // every conjugate gen·s·gen⁻¹ (and by gen⁻¹) under G's generators, by BFS.
    // The ordinary subgroup generated by a conjugation-closed set is normal, so
    // the multiplicative closure (done by PermutationGroup) is the normal closure.
    std::set<std::vector<int>> seen;
    std::vector<Permutation> seeds;
    std::vector<Permutation> frontier;
    for (const auto& s : generators) {
        if (seen.insert(s.array_form()).second) {
            seeds.push_back(s);
            frontier.push_back(s);
        }
    }

    const auto& gg = g.generators();
    while (!frontier.empty()) {
        std::vector<Permutation> next;
        for (const auto& p : frontier) {
            for (const auto& gen : gg) {
                for (Permutation c : {gen.compose(p).compose(gen.inverse()),
                                      gen.inverse().compose(p).compose(gen)}) {
                    if (seen.insert(c.array_form()).second) {
                        seeds.push_back(c);
    ...
    // PermutationGroup performs the multiplicative BFS closure on these seeds.
    return PermutationGroup(std::move(seeds));
}

bool is_simple(const PermutationGroup& g) {
    // A trivial group is not simple by SymPy's convention.
    if (g.order() <= 1) return false;
    const long full = g.order();
    // G is simple iff the normal closure of every non-identity element is all of
    // G (any proper nontrivial normal subgroup would contain such an element and
    // its whole closure, which would then be proper).
    for (const auto& x : g.elements()) {
        if (x.is_identity()) continue;
        if (normal_closure(g, {x}).order() != full) return false;
    }
    return true;
}

// Primary (prime-power) decomposition of a finite abelian group, matching
// SymPy's abelian_invariants: the sorted list of prime powers p^e of the primary
// cyclic factors. The group is described abstractly by its order `n` and a
// torsion oracle `torsion_count(m)` = |{ x : x^m = e }|.
//
// For each prime p | n, the number of cyclic p-factors of order ≥ p^k is
//   rank_k = log_p( |Q[p^k]| / |Q[p^{k-1}]| ).
// A factor has order exactly p^k for (rank_k − rank_{k+1}) of them.
namespace {
[[nodiscard]] std::vector<long> primary_decomposition(
    long n, const std::function<long(long)>& torsion_count) {
    if (n <= 1) return {};
    ...
    auto ipow = [](long base, int e) { long r = 1; for (int i = 0; i < e; ++i) r *= base; return r; };
    auto logp = [](long value, long p) { int e = 0; while (value > 1) { value /= p; ++e; } return e; };

    std::vector<long> factors;
    for (long p : primes) {
        std::vector<int> ranks;
        long prev = 1;  // |Q[p^0]| = 1 (identity only)
        for (int k = 1;; ++k) {
            long cur = torsion_count(ipow(p, k));
            if (cur == prev) break;
            ranks.push_back(logp(cur / prev, p));
            prev = cur;
        }
        for (std::size_t k = 0; k < ranks.size(); ++k) {
            int next = (k + 1 < ranks.size()) ? ranks[k + 1] : 0;
            int exact = ranks[k] - next;
            for (int i = 0; i < exact; ++i) factors.push_back(ipow(p, static_cast<int>(k + 1)));
        }
    }
...
std::vector<long> abelian_invariants(const PermutationGroup& g) {
    // The abelianization G/G' is abelian of order |G|/|G'|. Realize it concretely
    // as the set of cosets of the derived subgroup, each represented by its
    // canonical (lexicographically least) member, and decompose that finite
    // abelian group into prime-power invariants.
    PermutationGroup derived = derived_subgroup(g);
    std::set<std::vector<int>> deriv_set;
    for (const auto& d : derived.elements()) deriv_set.insert(d.array_form());

    // Canonical representative of the coset x·G': the lexicographically least
    // array form among { x·d : d ∈ G' }.
    auto coset_rep = [&](const Permutation& x) -> std::vector<int> {
        std::vector<int> best;
        for (const auto& d : derived.elements()) {
            std::vector<int> af = x.compose(d).array_form();
            if (best.empty() || af < best) best = std::move(af);
        }
        return best;
    };

    std::map<std::vector<int>, Permutation> reps;  // canonical rep → a representative
    for (const auto& x : g.elements()) {
        std::vector<int> rep = coset_rep(x);
        reps.emplace(rep, Permutation(rep));
    }

    const int degree = g.degree();
    const long n = static_cast<long>(reps.size());  // |G/G'|
    if (n <= 1) return {};

    const std::vector<int> id_rep = coset_rep(Permutation::identity(degree));

    // Torsion oracle in the quotient: |{ coset c : c^m = G' }|. Power a coset
    // representative and reduce the result back to its canonical coset.
    auto torsion_count = [&](long m) -> long {
        long count = 0;
        for (const auto& [rep, perm] : reps) {
            Permutation acc = Permutation::identity(degree);
            for (long i = 0; i < m; ++i) acc = acc.compose(perm);
            if (coset_rep(acc) == id_rep) ++count;
        }
        return count;
    };

    return primary_decomposition(n, torsion_count);
}
