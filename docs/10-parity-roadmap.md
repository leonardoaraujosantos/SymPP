...
| Last Gruntz mrv-set rewrite | `series/gruntz.py` | 1 wk | Medium | Most stages shipped; the previously-flagged `0·∞` divergent-exp case (eˣ(e^{1/x}−1)→∞) now evaluates, as do x^{1/x}→1, (1+1/x)ˣ→e. **Termination is now guaranteed** — the rewrite search is memoized and bounded by a work budget, so deeply nested exp/log towers return an honest nan instead of hanging (previously they ran unboundedly). Only a true general mrv-set rewrite for the *exact value* of that residual long tail remains |
