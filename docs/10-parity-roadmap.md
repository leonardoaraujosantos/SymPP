...
| 🟡 Multivariate `Poly` + Wang factorization | `polys/` | 2 wk | Medium | **Bivariate Wang shipped** — `factor()` handles genuinely-multivariate (symbolic-coefficient) inputs via content extraction over the second variable plus a quadratic-in-var solve through a perfect-square discriminant, every result verified to expand back to the input (x²y²−1→(xy−1)(xy+1), x²+xy+x+y→(x+1)(x+y), (x+1)²−y²→(x−y+1)(x+y+1), …; WANG-1). Remaining: higher-degree multivariate Hensel lifting and >2 variables |
