...
**Test surface to maintain**: 1525 cases / 5317 assertions (670
...
```
1525 tests / 5317 assertions  all passing (670 oracle-validated vs SymPy)
...
| 6  | Full Gruntz limit algorithm                          | 1 wk | High — **largely shipped** as composable leading-term stages (see `add-heavy-algorithm-engines` in `openspec/`): power-as-exp, dominant-term `∞−∞`, power continuity, log-exp reduction for nested transcendentals (`log x/exp(√(log x·log log x))→0`), numerically-verified small-angle/Maclaurin-head for `0·∞` analytic forms (`eˣ·(sin(1/x+e⁻ˣ)−sin(1/x))→1`), harmonic-number asymptotics, radical conjugate differences (`√(x²+x)−x`, `√(x+√x)−√x`), and a gamma-ratio asymptotic with Legendre/Gauss multiplication, exponential rates and unbalanced gamma powers (`Γ(2n)/nⁿ`, `Γ(2x)/Γ(x)²`). Remaining: a true mrv-set rewrite for the few cases the numeric check cannot certify, and full asymptotic-series depth. |
...
```
Where we are:        14/15 phases shipped, Phase 16 partial; 1525 tests
